...
export const executeProxiedRequest = async (
  ...
  // FORCE HYBRID MODE: Always 'hybrid' to ensure failover protection.
  const tokenMode = 'hybrid'; 
  ...
  if (specificToken) {
      ...
      // If the specific token provided matches the user's personal token, label it as "Personal" in logs
      ...
      // SCENARIO C: Hybrid "Bulletproof" Mode (Default & Enforced)
      // This logic tries Personal first, then Pool on Current Server, then Pool on Backup Servers.
