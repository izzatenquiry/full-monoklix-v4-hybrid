...
// Helper to get a random token from pool, excluding specific ones
const getRandomToken = (excludeTokens: string[] = []): string | null => {
    try {
        const tokensJSON = sessionStorage.getItem('veoAuthTokens');
        if (tokensJSON) {
            const sharedTokens: { token: string }[] = JSON.parse(tokensJSON);
            // Filter out excluded tokens
            const candidates = sharedTokens.filter(t => !excludeTokens.includes(t.token));
            
            if (candidates.length > 0) {
                const randomIndex = Math.floor(Math.random() * candidates.length);
                return candidates[randomIndex].token;
            } else if (sharedTokens.length > 0) {
                 // If all are excluded (rare), just pick any random one
                 const randomIndex = Math.floor(Math.random() * sharedTokens.length);
                 return sharedTokens[randomIndex].token;
            }
        }
    } catch (e) {
        console.warn("Could not pick random token", e);
    }
    return null;
};

// Helper to get a random server from pool
const getRandomServer = (): string => {
    if (PROXY_SERVER_URLS.length > 0) {
        return PROXY_SERVER_URLS[Math.floor(Math.random() * PROXY_SERVER_URLS.length)];
    }
    return 'https://s1.monoklix.com'; // Fallback
};
 ...
 * Implements a 4-Phase Strategic Retry Loop.
 */
export const generateVideo = async (
    ...
    if (currentUserJson) {
        ...
        if (u.personalAuthToken) currentToken = u.personalAuthToken;
    }
    const sessionServer = sessionStorage.getItem('selectedProxyServer');
    if (sessionServer) currentServer = sessionServer;

    let lastError: any;
    const usedTokens: string[] = [];
    if (currentToken) usedTokens.push(currentToken);

    // --- PHASE 1: INITIAL ATTEMPT (Current Token, Current Server) ---
    try {
        addLogEntry({ model, prompt, output: `Phase 1: Initial Attempt...`, tokenCount: 0, status: "Success" });
        return await attemptVideoGeneration(prompt, model, aspectRatio, resolution, image, currentToken, currentServer, onStatusUpdate);
    } catch (e: any) {
        lastError = e;
        console.warn(`[Video Gen] Phase 1 Failed: ${e.message}`);
        if (isSafetyError(e.message)) throw e; // Don't retry safety errors
    }

    // --- PHASE 2: SERVER HOP (Same Token, 3 Different Servers) ---
    // Only if we have a token to reuse
    if (currentToken) {
        for (let i = 1; i <= 3; i++) {
            const randomServer = getRandomServer();
            const msg = `Phase 2 (Attempt ${i}/3): Server Hop to ${randomServer.replace('https://', '')}...`;
            if (onStatusUpdate) onStatusUpdate(msg);
            addLogEntry({ model, prompt, output: msg, tokenCount: 0, status: "Success" });
            
            try {
                return await attemptVideoGeneration(prompt, model, aspectRatio, resolution, image, currentToken, randomServer, onStatusUpdate);
            } catch (e: any) {
                lastError = e;
                console.warn(`[Video Gen] Phase 2.${i} Failed: ${e.message}`);
                if (isSafetyError(e.message)) throw e;
            }
            await new Promise(resolve => setTimeout(resolve, 2000)); // Cooldown
        }
    }

    // --- PHASE 3: NEW IDENTITY 1 (New Token, 3 Different Servers) ---
    const newToken1 = getRandomToken(usedTokens);
    if (newToken1) {
        usedTokens.push(newToken1);
        for (let i = 1; i <= 3; i++) {
            const randomServer = getRandomServer();
            const msg = `Phase 3 (Attempt ${i}/3): New Token 1 on ${randomServer.replace('https://', '')}...`;
            if (onStatusUpdate) onStatusUpdate(msg);
            addLogEntry({ model, prompt, output: msg, tokenCount: 0, status: "Success" });

            try {
                return await attemptVideoGeneration(prompt, model, aspectRatio, resolution, image, newToken1, randomServer, onStatusUpdate);
            } catch (e: any) {
                lastError = e;
                console.warn(`[Video Gen] Phase 3.${i} Failed: ${e.message}`);
                if (isSafetyError(e.message)) throw e;
            }
            await new Promise(resolve => setTimeout(resolve, 2000));
        }
    } else {
        addLogEntry({ model, prompt, output: "Phase 3 Skipped: No spare tokens.", tokenCount: 0, status: "Success" });
    }

    // --- PHASE 4: NEW IDENTITY 2 (Another New Token, 3 Different Servers) ---
    const newToken2 = getRandomToken(usedTokens);
    if (newToken2) {
        usedTokens.push(newToken2);
        for (let i = 1; i <= 3; i++) {
            const randomServer = getRandomServer();
            const msg = `Phase 4 (Attempt ${i}/3): New Token 2 on ${randomServer.replace('https://', '')}...`;
            if (onStatusUpdate) onStatusUpdate(msg);
            addLogEntry({ model, prompt, output: msg, tokenCount: 0, status: "Success" });

            try {
                return await attemptVideoGeneration(prompt, model, aspectRatio, resolution, image, newToken2, randomServer, onStatusUpdate);
            } catch (e: any) {
                lastError = e;
                console.warn(`[Video Gen] Phase 4.${i} Failed: ${e.message}`);
                if (isSafetyError(e.message)) throw e;
            }
            await new Promise(resolve => setTimeout(resolve, 2000));
        }
