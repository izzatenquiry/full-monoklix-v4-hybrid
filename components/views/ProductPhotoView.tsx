...
const ProductPhotoView: React.FC<ProductPhotoViewProps> = ({ onReEdit, onCreateVideo, language, currentUser, onUserUpdate }) => {
  ...
  const generateOneImage = useCallback(async (index: number, attempt = 1, excludedServers: string[] = []) => {
    ...
    const isPersonalOnly = false; // Force Hybrid Mode

    let serverToUseUrl: string | undefined = undefined;

    if (!isPersonalOnly) {
        // Hybrid Mode: Randomize server selection to ensure load distribution and high availability.
    ...
    try {
        const { imageBase64, prompt } = await performGeneration(serverToUseUrl);
        ...
        if (!isNonRetriable) {
             // console.error(`Product Photo Generation Failed (Slot ${index + 1}, Attempt ${attempt} on ${serverToUseUrl})`);
        }
        
        if (!isPersonalOnly && attempt < 3 && !isNonRetriable && serverToUseUrl) {
